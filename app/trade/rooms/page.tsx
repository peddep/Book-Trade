  ...
  return (
    <>
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl font-bold text-[var(--text-heading)] mt-2 mb-1">🚪 {t('hub.rooms')}</h1>
        <p className="text-sm text-[var(--text-secondary)] mb-6">{t('room.desc')}</p>
          ...
          <>
            <div className="grid sm:grid-cols-2 gap-3 mb-5">
              <button onClick={() => { setMode(mode === 'create' ? null : 'create'); setErr(''); }}
                ...
                style={{ background: mode === 'create' ? 'var(--tint)' : 'var(--card)', border: `1px solid ${mode === 'create' ? '#A67C9C' : 'var(--border)'}` }}>
                <p className="font-bold text-[var(--text-heading)]">➕ {t('room.create')}</p>
                <p className="text-xs mt-1" style={{ color: '#f59e0b' }}>⭐ {t('room.createNote')}</p>
              </button>
              <button onClick={() => { setMode(mode === 'join' ? null : 'join'); setErr(''); }}
                className="p-5 rounded-2xl text-left"
                style={{ background: mode === 'join' ? 'var(--tint)' : 'var(--card)', border: `1px solid ${mode === 'join' ? '#A67C9C' : 'var(--border)'}` }}>
                <p className="font-bold text-[var(--text-heading)]">🔑 {t('room.join')}</p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">{t('room.codePlaceholder')}</p>
              </button>
            </div>
            ...
            {mode && (
              <div className="p-5 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                ...
                <p className="text-sm font-semibold text-[var(--text-mid)] mb-2">{t('hub.pickBook')}</p>
        ...
        {room && (
          <div className="p-5 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
              <div>
                <p className="text-xs text-[var(--text-secondary)]">{t('room.shareCode')}</p>
                <p className="font-mono text-2xl font-bold tracking-widest" style={{ color: '#986D8E' }}>{room.code}</p>
              </div>
              <span className="text-sm font-semibold text-[var(--text-mid)]">{t('room.members', { n: room.members.length, max: room.max })}</span>
            </div>

            <div className="flex flex-col gap-2 mb-4">
              {room.members.map(m => (
                <div key={m.user_id} className="flex items-center gap-3 p-2.5 rounded-xl" style={{ background: 'var(--card)' }}>
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-[var(--text-heading)] text-xs font-bold flex-shrink-0" style={{ background: m.avatar_color }}>
                    {m.name[0].toUpperCase()}
                  </div>
                  <span className="text-sm text-[var(--text-heading)] font-semibold">{m.name}</span>
                  <span className="ml-auto flex items-center gap-2 text-xs text-[var(--text-secondary)] min-w-0">
            ...
            {room.status === 'open' ? (
              <>
                {room.is_owner ? (
                  ...
                  <p className="text-center text-sm text-[var(--text-secondary)] py-2">{t('room.waitOwner')}</p>
                )}
              ...
              <div className="p-4 rounded-xl" style={{ background: '#dcfce7', border: '1px solid #10b981' }}>
                <p className="font-bold mb-1" style={{ color: '#10b981' }}>🎉 {t('room.done')}</p>
                {myResult?.received_title ? (
                  <>
                    <div className="flex items-center gap-3 mt-1">
                      <BookThumb coverUrl={myResult.received_cover_url} coverColor={myResult.received_color ?? 'var(--border)'} size={40} />
                      <p className="text-sm text-[var(--text-heading)] font-semibold">{bookTitle(myResult.received_title, myResult.received_title_en)}</p>
                    </div>
                    <p className="text-xs mt-1" style={{ color: '#059669' }}>{t('wb.meetHint')}</p>
                  </>
                ) : (
                  <p className="text-sm text-[var(--text-mid)]">{t('room.noTrade')}</p>
                )}
              </div>
            )}
          </div>
