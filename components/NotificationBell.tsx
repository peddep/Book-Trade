      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 sm:w-80 rounded-xl shadow-lg z-50 overflow-hidden bt-pop-in"
          ...
          <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: '1px solid var(--border)' }}>
            <p className="text-sm font-bold text-[var(--text-heading)]">{t('notif.title')}</p>
            {unread > 0 && (
          ...
          <div className="max-h-80 overflow-y-auto">
            {items.length === 0 ? (
              <p className="text-xs text-[var(--text-muted)] text-center py-8 px-4">{t('notif.empty')}</p>
            ) : (
              items.map(n => (
                <button
                  key={n.id}
                  onClick={() => openItem(n)}
                  className="block w-full text-left px-4 py-2.5 hover:bg-[var(--tint)]"
                  style={{ borderBottom: '1px solid var(--tint)', background: n.read ? undefined : 'var(--tint)' }}
                >
                  <div className="flex items-start gap-2.5">
                    <span className="text-base leading-none mt-0.5">{ICONS[n.kind] ?? '🔔'}</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs leading-snug text-[var(--text-heading)]">
                        {t(`notif.${n.kind}`, { actor: n.actor ?? '', subject: n.subject ?? '' })}
                      </p>
                      <p className="text-[10px] mt-0.5" style={{ color: 'var(--text-muted)' }}>{timeAgo(n.created_at, t, lang)}</p>
                    </div>
