...
export default function DonationCard({ userName }: { userName: string }) {
  ...
  return (
    <>
      <div className="p-5 rounded-2xl mb-6" style={{ background: 'linear-gradient(135deg, var(--card), var(--tint))', border: '1px solid var(--border)' }}>
        <h2 className="font-bold text-[var(--text-heading)] mb-3">{t('don.title')}</h2>
        {top.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)] mb-4">{t('don.none')}</p>
        ) : (
          <div className="flex flex-col gap-2 mb-4">
            {top.map((d, i) => (
              <div key={d.user_id} className="flex items-center gap-3 p-2 rounded-xl" style={{ background: i === 0 ? '#fef9c3' : 'var(--card)', border: '1px solid var(--tint)' }}>
                ...
                <span className="text-sm font-semibold text-[var(--text-heading)] flex-1 truncate">{d.name}</span>
                <span className="text-sm font-bold flex-shrink-0" style={{ color: '#986D8E' }}>฿{Number(d.total)}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {stage !== 'closed' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(46,16,101,0.45)' }}
          onClick={closeAll}>
          <div className="w-full max-w-sm rounded-2xl p-6 bt-pop-in" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
            onClick={e => e.stopPropagation()}>

            {stage === 'form' && (
              <>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-lg font-bold text-[var(--text-heading)]">💜 {t('don.modalTitle')}</p>
                  <button onClick={closeAll} className="w-8 h-8 rounded-full flex items-center justify-center text-[var(--text-secondary)] text-xl" style={{ background: '#f3f4f6' }}>✕</button>
                </div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">{t('don.bankName')}</label>
                <input value={bankName} onChange={e => setBankName(e.target.value)} maxLength={100}
                  placeholder={t('don.bankNamePlaceholder')}
                  className="w-full px-3 py-2 rounded-xl text-sm mb-3 text-[var(--text-heading)]" style={{ background: 'var(--tint)', border: '1px solid var(--border)', outline: 'none' }} />
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">{t('don.amount')}</label>
            ...
            {stage === 'qr' && (
              <>
                <p className="text-base font-bold text-[var(--text-heading)] text-center mb-3">{t('don.scanTitle', { amount })}</p>
                ...
                <p className="text-xs text-[var(--text-secondary)] text-center mb-3">{t('don.scanHint', { name: bankName })}</p>
                <button onClick={() => setStage('thanks')} className="w-full py-2 rounded-xl font-semibold text-sm" style={{ background: '#f3f4f6', color: 'var(--text-secondary)' }}>
                  {t('don.close')}
                </button>
              </>
            )}
