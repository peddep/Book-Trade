...
export default function DonationCard({ userName }: { userName: string }) {
  ...
  return (
    <>
      <div className="p-5 rounded-2xl mb-6" style={{ background: 'linear-gradient(135deg, #ffffff, #EFE3D0)', border: '1px solid #D9CAB3' }}>
        <h2 className="font-bold text-[#3D2A39] mb-3">{t('don.title')}</h2>
        {top.length === 0 ? (
          <p className="text-sm text-[#6b7280] mb-4">{t('don.none')}</p>
        ) : (
          <div className="flex flex-col gap-2 mb-4">
            {top.map((d, i) => (
              <div key={d.user_id} className="flex items-center gap-3 p-2 rounded-xl" style={{ background: i === 0 ? '#fef9c3' : '#ffffff', border: '1px solid #EFE3D0' }}>
                ...
                <span className="text-sm font-semibold text-[#3D2A39] flex-1 truncate">{d.name}</span>
      ...
      {stage !== 'closed' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(46,16,101,0.45)' }}
          onClick={closeAll}>
          <div className="w-full max-w-sm rounded-2xl p-6 bt-pop-in" style={{ background: '#ffffff', border: '1px solid #D9CAB3' }}
            onClick={e => e.stopPropagation()}>

            {stage === 'form' && (
              <>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-lg font-bold text-[#3D2A39]">💜 {t('don.modalTitle')}</p>
                ...
                <input value={bankName} onChange={e => setBankName(e.target.value)} maxLength={100}
                  ...
                  className="w-full px-3 py-2 rounded-xl text-sm mb-3 text-[#3D2A39]" style={{ background: '#EFE3D0', border: '1px solid #D9CAB3', outline: 'none' }} />
            ...
            {stage === 'qr' && (
              <>
                <p className="text-base font-bold text-[#3D2A39] text-center mb-3">{t('don.scanTitle', { amount })}</p>
                <div className="rounded-2xl overflow-hidden mb-3" style={{ border: '1px solid #D9CAB3' }}>
            ...
            {stage === 'thanks' && (
              <div className="text-center">
                ...
                <p className="text-lg font-bold text-[#3D2A39] mb-2">{t('don.thanksTitle')}</p>
